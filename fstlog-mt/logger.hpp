...
using log_level = decltype(std::declval<fstlog::logger_mt>().level());
...
public:
		...
		logbench::test_out_param& out_data)
	{
		...
		logger_instance_.new_buffer(buffer_size_ * init_data.thread_num);
...
private:
	...
	inline static fstlog::logger_mt logger_instance_{ core_instance_, "logger" };
};
