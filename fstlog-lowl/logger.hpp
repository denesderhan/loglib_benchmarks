...
public:
		...
		logbench::test_out_param& out_data)
	{
		...
		if (out_data.formatter_type == logbench::formatter_typ::txt) {
			...
			formatter = fstlog::formatter_txt_fast(log_templ_);
		...
		core_instance_.poll_interval(std::chrono::milliseconds{ 400 });
	}

	static void sys_stop() noexcept {
		core_instance_.poll_interval(std::chrono::milliseconds{0});
		core_instance_.stop();
	}
...
#define LOGBENCH_LOG_TRACE(logger, ...) logger.logger_impl().log<log_level::Trace, fstlog::log_policy_lowlatency, \
fstlog::ut_cast(fstlog::log_metaargs::File) | fstlog::ut_cast(fstlog::log_metaargs::Line)>( \
	__FILE__, __LINE__, __VA_ARGS__)

#define LOGBENCH_LOG_DEBUG(logger, ...) logger.logger_impl().log<log_level::Debug, fstlog::log_policy_lowlatency, \
fstlog::ut_cast(fstlog::log_metaargs::File) | fstlog::ut_cast(fstlog::log_metaargs::Line)>( \
	__FILE__, __LINE__, __VA_ARGS__)

#define LOGBENCH_LOG_INFO(logger, ...) logger.logger_impl().log<log_level::Info, fstlog::log_policy_lowlatency, \
fstlog::ut_cast(fstlog::log_metaargs::File) | fstlog::ut_cast(fstlog::log_metaargs::Line)>( \
	__FILE__, __LINE__, __VA_ARGS__)

#define LOGBENCH_LOG_WARN(logger, ...) logger.logger_impl().log<log_level::Warn, fstlog::log_policy_lowlatency, \
fstlog::ut_cast(fstlog::log_metaargs::File) | fstlog::ut_cast(fstlog::log_metaargs::Line)>( \
	__FILE__, __LINE__, __VA_ARGS__)

#define LOGBENCH_LOG_ERROR(logger, ...) logger.logger_impl().log<log_level::Error, fstlog::log_policy_lowlatency, \
fstlog::ut_cast(fstlog::log_metaargs::File) | fstlog::ut_cast(fstlog::log_metaargs::Line)>( \
	__FILE__, __LINE__, __VA_ARGS__)

#define LOGBENCH_LOG_FATAL(logger, ...) logger.logger_impl().log<log_level::Fatal, fstlog::log_policy_lowlatency, \
fstlog::ut_cast(fstlog::log_metaargs::File) | fstlog::ut_cast(fstlog::log_metaargs::Line)>( \
	__FILE__, __LINE__, __VA_ARGS__)
